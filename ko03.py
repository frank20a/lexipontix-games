...
try:
    ...
    mainImg = scaleLockAspect(pg.image.load(os.path.join(srcDir, 'img1.jpg')).convert(), 400)
    antennaImg = scaleLockAspect(pg.image.load(os.path.join(srcDir, 'img2.jpg')).convert_alpha(), 250)
    mouseImg = scaleLockAspect(pg.image.load(os.path.join(srcDir, 'img3.jpg')).convert_alpha(), 250)
...
class InfoWindow(tk.Tk):
    def __init__(self, title, msg):
        ...
        tk.Label(self, text='Software Development: Frank Fourlas', justify=tk.RIGHT).grid(row=1, column=0, sticky=tk.E, padx=15)
...
def drawCardText(scr, cont, posx, posy):
    for key in cont:
        if len(cont[key]) > 0:
            ...
            while len(txt):
                temp = []
                while len(' '.join(temp)) < 40 and len(txt) > 0: temp.append(txt.pop(0))
...
while running:
    ...
    screen.blit(logoTxt.render("Κο-03", True, colors['dark-grey']), (width-118, 32))
    ...
    t = titleTxt.render("Ποντικός Εναντίον Κέντρου Ελέγχου", True, colors['lime'])
    screen.blit(t, (width/2 - t.get_width()/2, int(85 * height / 1080)))
    ...
    screen.blit(antennaImg, (150, int(height/2 - antennaImg.get_height()/2 - 100)))
    screen.blit(mouseImg, (int(width - 150 - mouseImg.get_width()), int(height/2 - mouseImg.get_height()/2 - 100)))
    ringSurf = screen.blit(ringImg, (int(width - 275), int(3*height/4 - ringImg.get_height()/2)))
