...
try:
    ...
    glassesImg = scaleLockAspect(pg.image.load(os.path.join(srcDir, 'img2.png')).convert_alpha(), 175)
    ## Fonts
        ...
        for key in cont:
            if len(cont[key]) > 0:
                ...
                while len(txt):
                    ...
                    scr.blit(t, (width / 2 - t.get_width() / 2, posy))
                    posy += 32
    except TypeError:
        pass
...
while running:
    ...
    screen.blit(logoTxt.render("Κo-11", True, colors['dark-grey']), (width - 118, 32))
    ...
    t = titleTxt.render("Φορά ή Όχι Αλλόκοτα Γυαλιά", True, colors['lime'])
    ...
    screen.blit(glassesImg, (int(width / 7 - glassesImg.get_width() / 2), int(2 * height / 7 - glassesImg.get_height() / 2)))
